    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          ...
          'Plex import now marks fully-watched shows as COMPLETED when airingStatus is absent or ended and no upcoming episode is known — no prior airing refresh required.',
          'iOS text inputs in settings and forms no longer trigger Safari auto-zoom on focus; all controls render at 16 px or larger.',
          'Safe-area insets are applied to the bottom navigation bar so content is not hidden by the home indicator on iPhone X and newer.',
          'Discover tab bar no longer overflows on narrow screens; all three tabs remain visible and tappable.',
          'Watchlist filter and sort toolbar collapses to a compact single row on mobile with full touch-sized controls.',
          'Settings select dropdowns open correctly on first tap on iOS — no double-tap required.',
        ],
      },
    ],
  },
